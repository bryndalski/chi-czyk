...
const newRoom = {
  ...
  createUser(user, whoAmI, whoWantsToPlay) {
    ...
    if (user.nickname === whoAmI.nickname) {
      ...
      userTemplatContainer
        .querySelector("input")
        .addEventListener("change", this.iWantToPlay);
    }

    if (whoWantsToPlay.includes(user.nickname))
      //jeśli chce grać dostaje już zaznaczony
  ...
  async fetchUsers() {
    let findMe = await new Promise(async (success, er) => {
      let roomUsers = new serverOperation(
        null,
        null,
        config.fetchWaitingRoom,
        null
      );
      let respond = await roomUsers.fetchData();
      if ((await respond.players.length) !== this.userArray.length) {
        //Sprawdza czy został dodany nowy użytkownik żeby nie renderować na nowo
      ...
      if ((await respond.whoWantsToPlay) != [])
        //bez sensu mielić pętle jak nikt nie chce grać
        Object.keys(this.relationInputNickname).forEach((element, counter) => {
          this.relationInputNickname[
            element
          ].wantsToPlay = respond.whoWantsToPlay.includes(element)
            ? true
            : false; //obie obdpowiedzi takie same jeśli gra to gra a jeśli element != temu to  nie gra proste szybkie
          this.relationInputNickname[
            element
          ].input.checked = respond.whoWantsToPlay.includes(element)
            ? true
            : false;
        });
      if (respond.whoWantsToPlay.length >= 2 || this.userArray.length == 4)
        newRoom.sychAndStart(await respond);
      else success(await respond);
    });
    return findMe;
  },
